...
const result = rawContent
  ...
  .map((diffs) => {
    const reverseDiffs = diffs.reverse().map((diff) => diff.reverse())
    reverseDiffs[0].push(0)

    reverseDiffs.forEach((diff, index) => {
      if (index === reverseDiffs.length - 1) return
      reverseDiffs[index + 1].push(last(reverseDiffs[index + 1]) - last(diff))
    })

    return reverseDiffs.reverse().map((diff) => diff.reverse())
  })
  .map((newHistory) => newHistory[0][0])
