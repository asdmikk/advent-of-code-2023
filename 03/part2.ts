...
const gears = [...parts.values()].filter((part) => part.partNumbers.size === 2)

const gearRatioSum = gears.reduce((acc, gear) => {
  const gearRatio = [...gear.partNumbers].reduce((acc, number) => {
    return acc * number
  }, 1)
